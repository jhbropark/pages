...
| `varis-mark-{32,64,168,180,256,512}.png` | 래스터 마크 (투명 배경) | 없음 |

래스터는 전부 `varis-mark.svg` 를 해당 크기로 직접 벡터 래스터화한 것입니다.
큰 PNG 를 축소한 게 아니므로 어느 크기에서도 가장자리가 뭉개지지 않습니다.
다른 크기가 필요하면 축소하지 말고 SVG 에서 새로 뽑으세요.
