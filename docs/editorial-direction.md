...
균일하게 어두운 사진에 녹아웃을 하면 헤드라인이 사라지는데, 파일은 멀쩡히 나옵니다.
`_knockout_is_legible()` 이 **글자 면적의 25% 이상이 그라운드 대비 WCAG 3:1**
(큰 글씨 최소치)에 도달하는지 보고, 미달이면 스크림 방식으로 떨어뜨리고 기록합니다.

이 측정은 한 번 틀렸다가 실제 게시물에서 잡혔습니다. 첫 버전은 마스크 영역의
**평균 명도**를 그라운드와 빼서 **차이 52 이상**을 요구했는데, 2026-09-21 오후
카드가 그대로 통과했습니다.

| | 평균 | 옛 기준(차이≥52) | 3:1 넘는 면적 | 결과 |
|---|---:|:---:|---:|---|
| 09-21 오전 | 116 | 통과 (100) | **45%** | 녹아웃이 맞았음 |
| 09-21 오후 | 87 | 통과 (71) | **2.7%** | 검정 위 짙은 회색으로 나감 |

두 가지가 틀렸습니다. **뺄셈은 가독성 기준이 아닙니다** — 85 대 16은 차이가 71이라도
2.55:1로 3:1에 미달입니다. 그리고 **평균은 녹아웃을 대표하지 못합니다** — 녹아웃은
밝은 부분으로 읽히는데, 평면 워시와 진짜 사진이 평균 29 차이로 나란히 서 있었습니다.
면적 비율로 물으니 45% 대 2.7%로 갈라집니다.

남아 있던 버퍼 12장을 새 기준으로 다시 재니 **4장이 같은 결함으로 나갈 예정**이었습니다.
